...
function ConfirmForm() {
  ...
  useEffect(() => {
    if (!token_hash || type !== "invite") {
      setError("El link de invitación no es válido o ya fue usado.");
    }
  }, [token_hash, type]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // Verify OTP first to establish session
    const { error: otpErr } = await supabase.auth.verifyOtp({
      token_hash,
      type: "invite",
    });
    if (otpErr) {
      setError("El link expiró o ya fue usado. Pedí una nueva invitación al administrador.");
      setLoading(false);
      return;
    }
  ...
  if (done) {
    return (
      <div className="flex flex-col items-center gap-3 py-8 text-center">
        ...
        <p className="font-bold text-zinc-900 text-lg">¡Cuenta activada!</p>
    ...
    <div className="bg-white border border-zinc-200 rounded-2xl p-6 shadow-sm">
      ...
      {(!token_hash || type !== "invite") ? (
        <div className="text-center py-4">
          <p className="text-sm text-red-600 mb-4">El link de invitación no es válido o ya fue usado.</p>
          <Link href="/login" className="text-sm font-semibold" style={{ color: "#1957A6" }}>Ir al inicio de sesión</Link>
        </div>
      ) : (
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div>
            <label className="block text-xs font-semibold text-zinc-500 uppercase tracking-wide mb-1.5">Nueva contraseña</label>
              ...
              <>
                <div className="mt-2 flex items-center gap-2">
                  <div className="flex gap-1 flex-1">
                    {[0,1,2].map(i => <div key={i} className={`h-1 flex-1 rounded-full transition-colors ${i < strength ? (strength===3?"bg-emerald-500":strength===2?"bg-amber-400":"bg-red-400"):"bg-zinc-200"}`}/>)}
                  </div>
                  <span className="text-[10px] font-semibold text-zinc-400">{strength===3?"Fuerte":strength===2?"Regular":"Débil"}</span>
                </div>
                <div className="mt-1.5 flex flex-col gap-0.5">
                  {[{ok:password.length>=8,text:"Al menos 8 caracteres"},{ok:/[A-Z]/.test(password),text:"Una mayúscula"},{ok:/[0-9]/.test(password),text:"Un número"}].map(({ok,text})=>(
                    <p key={text} className={`text-[11px] flex items-center gap-1.5 ${ok?"text-emerald-600":"text-zinc-400"}`}><span>{ok?"✓":"·"}</span>{text}</p>
                  ))}
                </div>
              </>
            )}
          ...
          <button type="submit" disabled={loading || strength < 3 || !confirm}
            ...
            {loading ? (
              <><svg className="animate-spin" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg>Activando...</>
            ) : (
              <><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="20 6 9 17 4 12"/></svg>Crear contraseña y entrar</>
            )}
          </button>
        </form>
      )}
