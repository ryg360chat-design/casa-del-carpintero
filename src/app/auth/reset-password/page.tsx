...
function ResetForm() {
  ...
  useEffect(() => {
    if (!token_hash || type !== "recovery") {
      setError("El link de restablecimiento no es válido o ya fue usado.");
    }
  }, [token_hash, type]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (otpErr) {
      setError("El link expiró o ya fue usado. Solicitá un nuevo link.");
      setLoading(false);
      return;
    }
  ...
  if (done) {
    return (
      <div className="flex flex-col items-center gap-3 py-8 text-center">
        ...
        <p className="font-bold text-zinc-900 text-lg">¡Contraseña actualizada!</p>
    ...
    <div className="bg-white border border-zinc-200 rounded-2xl p-6 shadow-sm">
      ...
      {(!token_hash || type !== "recovery") ? (
        <div className="text-center py-4">
          ...
          <p className="text-sm text-red-600 mb-4">El link de restablecimiento no es válido o ya fue usado.</p>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <button type="submit" disabled={loading || strength < 3 || !confirm}
            ...
            {loading ? (
              ...
              <><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="20 6 9 17 4 12"/></svg>Guardar nueva contraseña</>
            )}
          </button>

          <p className="text-center text-xs text-zinc-400">
            <Link href="/auth/forgot" className="font-semibold hover:text-zinc-600" style={{ color: "#1957A6" }}>
