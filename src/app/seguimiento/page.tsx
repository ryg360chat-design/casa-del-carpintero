...
export default async function SeguimientoPage({
  ...
  return (
    <div className="min-h-screen" style={{ background: "linear-gradient(160deg, rgba(219,234,254,0.3) 0%, #fafaf9 40%)" }}>
      {/* Header */}
      <header className="border-b border-blue-100/60" style={{ background: "rgba(255,255,255,0.88)", backdropFilter: "blur(12px)" }}>
      ...
      <main className="max-w-xl mx-auto px-4 py-8">
        {/* Buscador */}
        <div className="bg-white border border-blue-100/60 rounded-2xl p-6 mb-6 shadow-sm shadow-blue-100/30">
          <div className="flex items-center gap-2 mb-1">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#1957A6" strokeWidth="2">
            ...
            {pedidos.map((p) => {
              ...
              return (
                <div key={p.id as string} className="bg-white border border-zinc-200/80 rounded-2xl overflow-hidden shadow-sm">
                  {/* Blue top accent */}
                  <div className="h-1 w-full" style={{ background: isCancelado ? "#ef4444" : isVendido ? "#0f766e" : isListo ? "#22c55e" : "linear-gradient(90deg, #1957A6, #267A8C)" }} />
                            ...
                            <div key={paso} className="flex items-center flex-1 last:flex-none">
                              <div className="flex flex-col items-center gap-1 shrink-0">
                                <div
                                  ...
                                  style={{
                                    background: done ? (isVendido ? "#0f766e" : isListo ? "#22c55e" : "#1957A6") : "#f4f4f5",
                                    color: done ? "white" : "#a1a1aa",
                                    boxShadow: current ? "0 0 0 3px rgba(25,87,166,0.2)" : "none",
                                  }}
                                >
                                  {completed ? (
                              ...
                              {idx < PASOS.length - 1 && (
                                <div className="flex-1 h-0.5 mb-4 mx-1" style={{ background: idx < pasoActual ? (isVendido ? "#0f766e" : isListo ? "#22c55e" : "#1957A6") : "#e4e4e7" }} />
                              )}
                            </div>
                          );
                        })}
                  ...
                  {!isCancelado && (
                    <div className="mx-5 mb-4 px-3.5 py-2.5 rounded-xl text-sm" style={{
                      background: isVendido ? "rgba(15,118,110,0.08)" : isListo ? "rgba(34,197,94,0.08)" : "rgba(25,87,166,0.07)",
                      border: `1px solid ${isVendido ? "rgba(15,118,110,0.2)" : isListo ? "rgba(34,197,94,0.2)" : "rgba(25,87,166,0.18)"}`,
                    }}>
                      <p className="font-semibold" style={{ color: isVendido ? "#0f766e" : isListo ? "#16a34a" : "#1957A6" }}>
        ...
        {!busqueda && (
          <div className="text-center py-12">
            <div className="w-16 h-16 rounded-2xl mx-auto mb-4 flex items-center justify-center" style={{ background: "linear-gradient(135deg, rgba(25,87,166,0.10), rgba(38,122,140,0.06))" }}>
              <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#1957A6" strokeWidth="1.5">
                <path d="M9 5H7a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2h-2"/>
                <rect width="6" height="4" x="9" y="3" rx="1"/>
                <path d="M9 12h6M9 16h4"/>
              </svg>
